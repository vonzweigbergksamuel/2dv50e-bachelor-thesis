...
### 1. Prepare and normalize the datasets
...
DeepFace require the following structure on the data in their databse:
...
_We need to place the images in the correct folder structure for the DeepFace framework to work. Known and unknown persons in the correct folder. The above is an example of how the known persons are placed in the database._

When our datasets is normalized we can split the dataset into know and unknown persons. We will use a 50/50 split to split the dataset into unknown and known persons. Then we will do another 50/50 split to split the perosns images into what already will be in the datase for the known persons and 50% of the images will go to the test set. Scikit-learn will be used for the splitting.

known group: is persons that already have at least 1 image in the database.
unknown group: is persons that do not have any images in the database.
